	      ...
	      <div class="modal-body">
				<form id="addEmpForm" class="form-horizontal">
				  <div class="form-group">
				    ...
				    <div class="col-lg-6">
				      <input type="text" class="form-control" id="empName" name="empName" placeholder="张三">
				      <span id="empNameError" style="display:none">必须是2-5位中文或6-16位英文和数字</span>
				    </div>
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-group">
				    ...
				    <div class="col-lg-6">
				      <input type="text" class="form-control" id="email" name="email" placeholder="123@hr.com">
	      ...
	      <div class="modal-body">
				<form id="updateEmpForm" class="form-horizontal">
				  <div class="form-group">
				    <label class="col-sm-2 control-label">员工名</label>
				    <div class="col-lg-6">
				      <input type="text" class="form-control-static" id="update_EmpName" name="empName">
				    </div>
				  </div>
				  <div class="form-group">
				    <label class="col-sm-2 control-label">性别</label>
	...
	//构建分页表格
		function build_emps_table(result){
			...
			$.each(emps,function(index,item){
				...
				var opeTd = $("<td></td>").append(btnEdit).append(" ").append(btnDelete);
		...
		function build_page_info(result){
			...
			var pageSta = $("<span></span>").append("当前是"+pageInfo.pageNum+"页,共有"+pageInfo.pages+"页，共计"+pageInfo.total+"条记录");
		...
		function build_page_nav(result){
		    ...
		    if(pageInfo.hasPreviousPage==false){
		    	  var firstPageBtn = $("<li></li>").addClass("disabled").append($("<a></a>").append("首页").attr("href","#"));
				  var prePageBtn = $("<li></li>").addClass("disabled").append($("<a></a>").append("&laquo;").attr("href","#"));	
		    }else{
			    var firstPageBtn = $("<li></li>").append($("<a></a>").append("首页").attr("href","#"));
			 	var prePageBtn = $("<li></li>").append($("<a></a>").append("&laquo;").attr("href","#"));
		    }
		    //为元素添加翻页事件
		    ...
		    if(pageInfo.hasNextPage==false){
		    	var nextPageBtn = $("<li></li>").addClass("disabled").append($("<a></a>").append("&raquo;").attr("href","#"));
			    var lastPageBtn = $("<li></li>").addClass("disabled").append($("<a></a>").append("末页").attr("href","#"));
		    }else{
				var nextPageBtn = $("<li></li>").append($("<a></a>").append("&raquo;").attr("href","#"));
			    var lastPageBtn = $("<li></li>").append($("<a></a>").append("末页").attr("href","#"));
		    }
		    //为元素添加翻页事件
		    ...
		    $("#pagination").append(firstPageBtn).append(prePageBtn).append(nextPageBtn).append(lastPageBtn);
			$.each(pageInfo.navigatepageNums,function(index,item){
				if(pageInfo.pageNum==item){
					var pagesBtn = $("<li></li>").addClass("active").append($("<a></a>").append(item).attr("href","#"));
				}else{
					var pagesBtn = $("<li></li>").append($("<a></a>").append(item).attr("href","#"));
				}
		...
		function build_dept_select(result){
			...
			$.each(departments,function(index,item){
				$("<option></option>").append(item.name).attr("value",item.id).appendTo($("#deptment"));
			});
		}
		//校验添加表单中的员工数据
		...
		function build_update_dept_select(result){
			...
			$.each(departments,function(index,item){
				$("<option></option>").append(item.name).attr("value",item.id).appendTo($("#update_deptment"));
			});
		}
	</script>
</body>
