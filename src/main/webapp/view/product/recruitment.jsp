...
<body>
	...
	<script>
																...
																$(infoBtn)
																		.click(
																				function() {
																					$
																							.ajax({
																								url : "${APP_PATH}/getRecruitmentTaskById/"
																										+ item.id,
																								type : "GET",
																								success : function(
																										result) {
		...
		$("#save_recruit_data").click(function() {
		...
		function buildRecruitInfo(recruit) {
			...
			var titleP = $("<p></p>").append("标题:" + recruit.name);
			var destinationP = $("<p></p>").append("目的:" + recruit.destination);
			var principalP = $("<p></p>").append("负责人:" + recruit.principal);
			var timeP = $("<p></p>").append(
					"时间:" + Millisecond2Date(recruit.time));
			var addressP = $("<p></p>").append("地点:" + recruit.address);
			var employeeinneedP = $("<p></p>").append(
					"需求:" + recruit.employeeinneed);
			$("#recruitmentInfo").append(titleP).append(destinationP).append(
					principalP).append(timeP).append(addressP).append(
					employeeinneedP);
